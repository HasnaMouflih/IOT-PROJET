...
void loop() {
  ...
  // Exemple: envoyer un message de test toutes les 10s
  static unsigned long lastTelemetry = 0;
  if (mqttConnected && millis() - lastTelemetry >= 10000) {
    String message = "{\"deviceId\":\"" + deviceId + "\", \"status\":\"ok\"}";
    mqttClient.publish(telemetryTopic.c_str(), message.c_str());
